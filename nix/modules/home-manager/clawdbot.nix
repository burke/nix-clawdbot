...
in {
  ...
  config = lib.mkIf (cfg.enable || cfg.instances != {}) {
    ...
    home.activation.clawdbotDirs = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      mkdir -p ${lib.concatStringsSep " " (lib.concatMap (item: item.dirs) instanceConfigs)}
      ${lib.optionalString (pluginStateDirsAll != []) "mkdir -p ${lib.concatStringsSep " " pluginStateDirsAll}"}
    '';

    home.activation.clawdbotConfigFiles = lib.hm.dag.entryAfter [ "clawdbotDirs" ] ''
      set -euo pipefail
      ${lib.concatStringsSep "\n" (map (item: "ln -sfn ${item.configFile} ${item.configPath}") instanceConfigs)}
    '';
